...
from zhiliang import __app_name__, __version__
from zhiliang.config import DEFAULT_PORT, STATIC_DIR, ensure_dirs
from zhiliang.engine.excel import content_disposition, download_name, export_xlsx
from zhiliang.engine.quality import add_inspection, add_issue, empty_project, load_catalog, set_issue_status
from zhiliang.store import attach_stats, get_project, load_workspace, save_workspace
...
def _issue(project: dict[str, Any], issue_id: str) -> dict[str, Any]:
    for item in project.get("issues") or []:
        if item["id"] == issue_id:
            return item
    raise HTTPException(404, "未找到问题")
...
async def create_issue(project_id: str, body: IssueBody) -> dict[str, Any]:
    data = load_workspace(_today())
    project = _project(data, project_id)
    try:
        add_issue(project, body.model_dump(), _today())
    except KeyError as exc:
...
@app.post("/api/projects/{project_id}/issues/{issue_id}/status")
async def change_status(project_id: str, issue_id: str, body: StatusBody) -> dict[str, Any]:
    data = load_workspace(_today())
    project = _project(data, project_id)
    issue = _issue(project, issue_id)
    try:
        set_issue_status(issue, body.status, body.model_dump(), _today())
...
async def delete_issue(project_id: str, issue_id: str) -> dict[str, Any]:
    data = load_workspace(_today())
    project = _project(data, project_id)
    project["issues"] = [i for i in project.get("issues") or [] if i["id"] != issue_id]
    save_workspace(data)
    return attach_stats(data, _today())
...
async def download_xlsx(project_id: str) -> Response:
    ...
    filename = download_name(project)
    return Response(
        content,
