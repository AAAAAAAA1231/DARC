...
from hub.config import DEFAULT_HOST, DEFAULT_PORT, STATIC_DIR, ensure_dirs
...
def main() -> None:
    ...
    ensure_dirs()
    parser = argparse.ArgumentParser(description="工程工作台")
    ...
    print("=" * 50, flush=True)
    print("工程工作台正在启动，请不要关闭这个黑窗口。", flush=True)
    print(f"启动后请打开：{url}", flush=True)
    print("模块：清标 / 安全 / 进度 / 质量 / 成本 / 技术标", flush=True)
