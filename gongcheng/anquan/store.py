...
from anquan.config import DATA_DIR, WORKSPACE_FILE, ensure_dirs
from anquan.engine.safety import demo_project, empty_project, enrich_project
...
def _strip(project: dict[str, Any]) -> dict[str, Any]:
    keep_proj = {
        "id", "name", "location", "manager", "safety_lead", "supervisor",
        "specialty", "notes", "hazards", "inspections",
    }
    drop = {"overdue", "overdue_days", "remain_days", "closed", "loop_step", "loop_label", "stats"}
    out = {k: project.get(k) for k in keep_proj}
    out["hazards"] = [{k: v for k, v in (i or {}).items() if k not in drop} for i in project.get("hazards") or []]
