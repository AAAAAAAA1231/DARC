...
async def fetch_new_protocols(lookback_days: int = 21, limit: int = 40) -> list[dict[str, Any]]:
    ...
    for p in rows:
        ...
        chain = chains[0] if chains else (p.get("chain") or "多链")
        ...
        items.append(
            {
                ...
                "name": name,
                "kind": "新项目上线",
                "chain": chain if isinstance(chain, str) else "多链",
                "text": f"{desc} · 分类 {p.get('category') or '-'} · TVL ${float(p.get('tvl') or 0):,.0f}",
                "url": url or tw,
                "twitter": tw,
                "created_at": created,
                "source": "新协议",
                "source_kind": "live",
                "price_usd": None,
                "extra": {"category": p.get("category"), "tvl": p.get("tvl"), "twitter": handle},
            }
        )
...
async def fetch_pre_tge_projects(limit: int = 24) -> list[dict[str, Any]]:
    """Unissued / funding-stage projects that may open whitelist, testnet, or TGE."""
    payload = await get_json(CRYPTORANK_COINS, params={"lifeCycle": "funding", "limit": 40}, timeout=20.0)
        ...
        items.append(
            {
                ...
                "kind": kind,
                "chain": str(cat or "未知"),
                "text": f"尚未进入交易阶段，适合跟官网/X 上的白名单、测试网、TGE 窗口。类型 {c.get('type') or '-'}",
                "url": url,
                "twitter": "",
                ...
                "source_kind": "live",
                "price_usd": None,
                "extra": {"lifeCycle": c.get("lifeCycle"), "type": c.get("type")},
            }
        )
        if len(items) >= limit:
            break
