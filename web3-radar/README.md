...
Windows 桌面终端（也可在本机用 Python 直接运行）。覆盖合约综合研判、meme 币监控、大使招募、打新、高融资空投，以及 OKX 等 Web3 钱包连接与确认队列。
...
1. **合约分析**  
   ...
   以初始份额为 Dirichlet 先验，做 **100 万次** 权重模拟。拟合按 **ATR 止盈止损路径的 R 倍数** 计分（不是简单的 8 根收盘涨跌），对收益期望最高的分位做加权平均，得到综合分，结论为 **涨 / 跌 / 观望**。  
   实盘层：高波动标的自动加宽止损；按 **1R 等风险** 给建议仓位（HYPE 与 ETH 不要按名义金额均分）；浮盈 1R 减仓 40% 并移至保本，1.5R 后 ATR 追踪；同屏最多 3 仓、同向最多 2 仓。趋势指标分歧时降为观望。

2. **Meme 币**  
   一个月一买：成功率优先，还要留倍数。只挑活过 3 天、池子够出、市值还小、且有 X/社区热度的 meme。**2 倍卖掉 35%** 锁定胜利，5 倍再减 25%，剩下拿到本月。前 24 小时 −20% 当撤池。单笔本金 1%，最多 1 仓。宁可不买也不接飞刀。
...
6. **钱包**  
   浏览器打开连接页，对接 OKX / MetaMask / Rabby 注入钱包。可把空投、打新、meme、合约信号加入队列。  
...
本工具提供研究与信息聚合，不是投资建议。meme 币、打新、合约均有极高风险。自动队列默认仍需钱包二次确认，请勿对未知合约无限授权。
