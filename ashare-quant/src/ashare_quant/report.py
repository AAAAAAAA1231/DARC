...
def write_report(out: Path, snapshot: dict, ideas: pd.DataFrame, wf, mc, bt) -> Path:
    ...
    lines = [
        ...
        "",
        f"- 信号日: **{snapshot.get('asof')}**",
        f"- 行情时刻: {snapshot.get('quote_time') or '-'}",
        f"- 数据来源: {snapshot.get('data_source_cn') or snapshot.get('data_source') or '-'}",
        f"- 行情说明: {snapshot.get('quote_note') or ''}",
