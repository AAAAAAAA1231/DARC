...
def layout_floor(room_w: float, room_d: float, tw: float, th: float, grout: float, pattern: str = "straight") -> dict[str, Any]:
    if pattern == "herringbone":
        ...
        return _pack("herringbone", tw, th, grout, gx, gy, tiles, extra="人字/鱼骨按正铺分格示意，现场按样板放线")
    if pattern == "diagonal":
...
def wet_floor_features(room_w: float, room_d: float, openings: list | None = None, kind: str = "卫生间") -> dict[str, Any]:
    """GB 55038-2025 4.1.9 地漏+1%坡；4.1.12 门口高差≤15mm 斜坡过渡."""
    doors = [o for o in (openings or []) if getattr(o, "kind", "door") == "door"]
    door = doors[0] if doors else None
    dx, dy = room_w * 0.68, room_d * 0.72
    ramp = None
    if door:
        wall = getattr(door, "wall", "S")
        off = float(getattr(door, "offset", 0))
        dw = float(getattr(door, "width", 0.7))
        if wall == "S":
            ramp = {"x": off, "y": 0.0, "w": dw, "d": 0.30, "drop_mm": 15}
            dy = min(room_d - 0.35, max(0.40, room_d * 0.75))
            dx = min(max(room_w * 0.72, 0.40), room_w - 0.35)
        elif wall == "N":
            ramp = {"x": off, "y": room_d - 0.30, "w": dw, "d": 0.30, "drop_mm": 15}
            dy = max(0.35, room_d * 0.25)
            dx = min(max(room_w * 0.72, 0.40), room_w - 0.35)
        elif wall == "W":
            ramp = {"x": 0.0, "y": off, "w": 0.30, "d": dw, "drop_mm": 15}
            dx = min(room_w - 0.35, max(0.40, room_w * 0.75))
            dy = min(max(room_d * 0.65, 0.40), room_d - 0.35)
        else:
            ramp = {"x": room_w - 0.30, "y": off, "w": 0.30, "d": dw, "drop_mm": 15}
            dx = max(0.35, room_w * 0.25)
            dy = min(max(room_d * 0.65, 0.40), room_d - 0.35)
    else:
        ramp = {"x": (room_w - 0.70) / 2, "y": 0.0, "w": 0.70, "d": 0.30, "drop_mm": 15}
    corners = [(0.0, 0.0), (room_w, 0.0), (0.0, room_d), (room_w, room_d)]
    dmax = max(math.hypot(x - dx, y - dy) for x, y in corners)
    full_fall = dmax * 0.01
    return {
        "drain": {"x": round(dx, 3), "y": round(dy, 3)} if kind == "卫生间" else None,
        "slope_pct": 1.0 if kind == "卫生间" else 0.0,
        "full_room_fall_mm": round(full_fall * 1000, 1),
        "local_slope": bool(kind == "卫生间" and full_fall > 0.015 + 1e-9),
        "door_step_mm": 15,
        "ramp": ramp,
        "kind": kind,
    }
